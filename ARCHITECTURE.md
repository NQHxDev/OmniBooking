...
- **End-to-End Idempotency (Idempotent Consumer)**:
   ...
   - **Atomic Claim-then-Process Pattern**: To prevent race conditions in highly concurrent environments (e.g., when partition rebalancing or parallel execution triggers duplicate consumption simultaneously), the check-then-act pattern has been upgraded to an atomic "Claim-then-Process" mechanism.
      - **Atomic Event Claim**: At the start of consumption, the consumer attempts to claim the event by inserting a record into the deduplication store (`processed_events`) within a separate transaction (`REQUIRES_NEW`). If the insert succeeds (database unique key constraint `(event_id, consumer_group)` is satisfied), the event is successfully claimed and processing continues. If a `DataIntegrityViolationException` occurs, the event is immediately identified as a duplicate and skipped safely.
      - **Transactional Release on Failure**: If the business logic or downstream processing fails, the transaction is rolled back, and the consumer explicitly deletes the claim record from `processed_events` in a separate transaction (`REQUIRES_NEW`). This allows Kafka to safely retry processing the event in subsequent attempts, avoiding permanently locked events that fail transiently.
