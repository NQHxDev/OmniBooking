...
- **CSRF Cookie Integration & Rotation**: The `csrf_token` cookie is managed with SameSite=Lax but is configured with `httpOnly(false)`. This allows Next.js/client-side JS to read the cookie value and send it back as the `X-CSRF-Token` header, matching the backend's Double Submit Cookie verification strategy. The token is dynamically rotated on key lifecycle events (login, logout, and token refresh). A bootstrap endpoint `GET /auth/csrf` is provided to fetch the CSRF token on startup or after expiration.
