...
import { LayoutDashboard, Building2, Users, BarChart3, Settings, Home } from "lucide-react";
...
export default function DashboardSidebar() {
   const t = useTranslations("Owner.sidebar");
   const pathname = usePathname();

   return (
      <aside className="fixed left-0 top-0 hidden h-screen w-64 border-r border-zinc-200 bg-white lg:block z-20 font-sans">
         <div className="flex h-full flex-col p-6">
            <div className="mb-10 px-2">
               ...
               <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#dc3545] mt-1">
                  Admin Panel
               </p>
            </div>
            ...
            <nav className="flex-1 space-y-1">
               {MENU_ITEMS.map((item) => {
                  ...
                  return (
                     ...
                     >
                        <item.icon
                           className={`h-5 w-5 ${isActive ? "text-rose-600" : "text-zinc-400"}`}
                        />
                        {t(item.labelKey)}
                     </Link>
                  );
