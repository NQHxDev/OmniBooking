...
import { getWebUrl, getOwnerUrl } from "@omnibooking/shared";
...
export async function proxy(request: NextRequest) {
   ...
   const ownerUrl = getOwnerUrl(browserUrl);
   const callbackUrl = `${ownerUrl}${pathname}`;
   ...
   if ((!token || isTokenExpired(token)) && refreshToken) {
      try {
         ...
         console.error("Silent token refresh in owner middleware failed:", err);
      }
   }
   ...
   try {
      ...
      const isAdmin = roles && (roles.includes("ROLE_ADMIN") || roles.includes("ROLE_OWNER"));
      ...
      // Run next-intl middleware for locale handling and redirects
