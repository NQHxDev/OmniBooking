...
export default function Navbar() {
   ...
   const isAdmin = user?.roles?.includes("ROLE_ADMIN") || user?.roles?.includes("ROLE_OWNER");
                  ...
                  {!mounted ? (
                     ...
                     <>
                        {isLoggedIn && isPartner ? (
                           <button
                              onClick={() => {
                                 window.location.href = "/api/auth/partner-login";
                              }}
                              className="hidden lg:flex items-center justify-center gap-2 text-[13px] font-semibold hover:bg-white/10 px-3 py-1.5 rounded-md transition-colors min-w-35 text-center shrink-0 cursor-pointer"
                           >
                                 ...
                                 <div className="absolute right-0 z-50 mt-2.5 w-56 origin-top-right rounded-xl bg-white text-zinc-900 shadow-2xl ring-1 ring-black ring-opacity-5 focus:outline-none animate-in fade-in zoom-in-95 duration-200 overflow-hidden">
                                    <div className="py-1.5">
                                       {isAdmin && (
                                          <DropdownItem
                                             icon={
                                                <ShieldCheck className="h-4.5 w-4.5 text-rose-500 animate-pulse" />
                                             }
                                             label={t("adminDashboard")}
                                             href="/api/auth/owner-login"
                                          />
                                       )}
                                       <DropdownItem
   ...
   if (isApi) {
      return (
         <button
            ...
            className="flex w-full items-center gap-3 px-4 py-2.5 text-[13px] font-medium text-zinc-700 hover:bg-zinc-50 transition-colors cursor-pointer text-left"
         >
