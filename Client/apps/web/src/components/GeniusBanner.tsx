   ...
   return (
      <div className="mt-6 flex flex-col md:flex-row items-center gap-8 rounded-2xl border border-zinc-200 p-8 shadow-[0_4px_20px_rgba(0,0,0,0.03)] bg-white transition-all hover:shadow-[0_8px_30px_rgba(0,0,0,0.06)] overflow-hidden relative group min-h-[160px]">
         ...
         <div className="flex-1 text-center md:text-left z-10 w-full">
            {!mounted ? (
               // Skeleton khi đang nạp trang để tránh nhảy nội dung
               <div className="space-y-3">
                  <div className="h-7 w-64 bg-zinc-100 rounded-lg animate-pulse mx-auto md:mx-0"></div>
                  <div className="h-5 w-full max-w-md bg-zinc-50 rounded-lg animate-pulse mx-auto md:mx-0"></div>
                  <div className="flex gap-4 justify-center md:justify-start mt-4">
                     <div className="h-10 w-32 bg-zinc-100 rounded-xl animate-pulse"></div>
                     <div className="h-10 w-32 bg-zinc-100 rounded-xl animate-pulse"></div>
                  </div>
               </div>
            ) : isLoggedIn ? (
