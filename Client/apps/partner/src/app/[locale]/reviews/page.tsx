...
import PartnerReviewList from "@/components/partner/PartnerReviewList";
import { getTranslations } from "next-intl/server";

export async function generateMetadata() {
   const t = await getTranslations("Partner.reviews");
   return {
      title: t("metaTitle"),
...
export default async function PartnerReviewsPage() {
   ...
   if (!cookieStore.get("session_id") && !cookieStore.get("refresh_token")) {
      redirect(`${webUrl}/auth/login?callbackUrl=${partnerUrl}/reviews`);
   }
      ...
      <div className="min-h-screen bg-zinc-50/50 font-sans">
         ...
         <main className="lg:pl-64">
            ...
            <div className="p-8">
               ...
               <PartnerReviewList initialProperties={properties} />
            </div>
         </main>
      </div>
   );
